import os
import json
import hashlib
import re
from datetime import datetime, timezone

import feedparser
from google import genai


ARTICLES_FILE = "articles.json"
SOURCES_FILE = "sources.json"

MAX_ARTICLES_PER_SOURCE = 10
MAX_TOTAL_ARTICLES = 30


def load_json(filename, default):
    if not os.path.exists(filename):
        return default

    try:
        with open(filename, "r", encoding="utf-8") as f:
            return json.load(f)
    except Exception:
        return default


def save_json(filename, data):
    with open(filename, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=2)


def clean_text(text):
    if not text:
        return ""

    text = re.sub(r"<[^>]+>", " ", text)
    text = re.sub(r"\s+", " ", text)

    return text.strip()


def make_id(title, url):
    value = f"{title}|{url}".encode("utf-8")
    return hashlib.sha256(value).hexdigest()[:20]


def load_sources():
    sources = load_json(SOURCES_FILE, [])

    valid = []

    for source in sources:
        if not source.get("reuse_policy_checked"):
            continue

        if not source.get("allow_publish"):
            continue

        if not source.get("feed_url"):
            continue

        valid.append(source)

    return valid


def fetch_feed(source):
    print(f"Reading: {source['name']}")

    feed = feedparser.parse(source["feed_url"])

    articles = []

    for item in feed.entries[:MAX_ARTICLES_PER_SOURCE]:

        title = clean_text(item.get("title", ""))
        link = item.get("link", "").strip()

        description = clean_text(
            item.get("summary", "")
            or item.get("description", "")
        )

        if not title or not link:
            continue

        article_id = make_id(title, link)

        published = (
            item.get("published", "")
            or item.get("updated", "")
            or ""
        )

        articles.append({
            "id": article_id,
            "source": source["name"],
            "source_website": source.get("website", ""),
            "source_url": link,
            "feed_title": title,
            "feed_summary": description[:5000],
            "published": published,
            "category": source.get("category", "General"),
            "allow_ai_rewrite": source.get(
                "allow_ai_rewrite",
                False
            )
        })

    return articles


def build_prompt(article):

    return f"""
You are the editorial AI for Snippet24.

IMPORTANT COPYRIGHT RULES:

1. Do NOT copy sentences from the source.
2. Do NOT reproduce the source article.
3. Do NOT pretend Snippet24 is the original publisher.
4. Create genuinely original wording.
5. Use ONLY facts explicitly present in the supplied RSS information.
6. Do NOT add facts from your own knowledge.
7. If the supplied information is insufficient, say so.
8. Do not invent names, numbers, dates, quotes or events.
9. Do not create direct quotations.
10. The source URL must remain unchanged.

The source is:

Source:
{article["source"]}

Original RSS headline:
{article["feed_title"]}

RSS summary:
{article["feed_summary"]}

Original source URL:
{article["source_url"]}

Return ONLY valid JSON.

Required structure:

{{
  "headline_1": "",
  "headline_2": "",
  "headline_3": "",

  "summary": "",

  "deep_dive": [
    "",
    "",
    "",
    "",
    "",
    "",
    "",
    "",
    "",
    "",
    "",
    "",
    "",
    "",
    "",
    "",
    "",
    "",
    "",
    "",
    "",
    "",
    "",
    "",
    "",
    "",
    "",
    "",
    "",
    "",
    ""
  ],

  "risk_level": "AUTO",
  "risk_reasons": []
}}

HEADLINES:
Create three substantially different original headlines.

SUMMARY:
Write a concise original summary based only on the supplied RSS information.

DEEP DIVE:
Write exactly 30 short numbered-information lines.

The 30 lines must NOT invent additional facts.

If there are not enough facts to produce 30 factual lines, repeat the limitation in different useful ways rather than inventing information.

RISK LEVEL:

Use AUTO only when:
- information is straightforward;
- no allegations are being made;
- no private personal information appears;
- no obvious defamation issue exists;
- no medical emergency advice is involved;
- no unverified accusation appears.

Use REVIEW for:
- allegations;
- crime accusations;
- political controversy;
- named individuals accused of wrongdoing;
- deaths or serious injuries;
- medical claims;
- financial claims;
- potentially defamatory statements;
- uncertain factual information;
- sensitive personal information.

Use BLOCK for:
- clearly unsafe content;
- private personal information;
- content that appears malicious or fabricated;
- instructions for wrongdoing;
- content that should not be published.

Never use a source quote.
"""


def fallback_article(article):
    summary = article["feed_summary"]

    if not summary:
        summary = (
            "The available RSS information does not contain "
            "enough detail for a full summary."
        )

    lines = []

    for i in range(30):
        if i == 0:
            lines.append(
                f"The source reports: {summary[:250]}"
            )
        else:
            lines.append(
                "Additional verified detail is not available "
                "in the supplied RSS information."
            )

    return {
        "headline_1": article["feed_title"],
        "headline_2": f"Latest update: {article['feed_title']}",
        "headline_3": f"What we know about {article['feed_title']}",
        "summary": summary[:1000],
        "deep_dive": lines,
        "risk_level": "REVIEW",
        "risk_reasons": [
            "AI generation was unavailable."
        ]
    }


def generate_ai(article):

    api_key = os.getenv("GEMINI_API_KEY")

    if not api_key:
        print("No Gemini API key. Using safe fallback.")
        return fallback_article(article)

    try:

        client = genai.Client(api_key=api_key)

        response = client.models.generate_content(
            model="gemini-2.5-flash",
            contents=build_prompt(article)
        )

        text = response.text.strip()

        if text.startswith("```"):
            text = re.sub(
                r"^```json\s*",
                "",
                text,
                flags=re.IGNORECASE
            )

            text = re.sub(
                r"\s*```$",
                "",
                text
            )

        result = json.loads(text)

        headlines = [
            result.get("headline_1", "").strip(),
            result.get("headline_2", "").strip(),
            result.get("headline_3", "").strip()
        ]

        deep_dive = result.get("deep_dive", [])

        if len(deep_dive) != 30:
            raise ValueError(
                "Deep dive must contain exactly 30 lines."
            )

        if not all(headlines):
            raise ValueError(
                "Missing headlines."
            )

        risk_level = result.get(
            "risk_level",
            "REVIEW"
        ).upper()

        if risk_level not in [
            "AUTO",
            "REVIEW",
            "BLOCK"
        ]:
            risk_level = "REVIEW"

        return {
            "headline_1": headlines[0],
            "headline_2": headlines[1],
            "headline_3": headlines[2],
            "summary": result.get(
                "summary",
                ""
            ).strip(),

            "deep_dive": [
                str(x).strip()
                for x in deep_dive
            ],

            "risk_level": risk_level,

            "risk_reasons": result.get(
                "risk_reasons",
                []
            )
        }

    except Exception as error:

        print(
            f"AI generation failed: {error}"
        )

        return fallback_article(article)


def process():

    sources = load_sources()

    print(
        f"Approved sources: {len(sources)}"
    )

    existing = load_json(
        ARTICLES_FILE,
        []
    )

    if not isinstance(existing, list):
        existing = []

    existing_ids = {
        item.get("id")
        for item in existing
        if item.get("id")
    }

    new_articles = []

    for source in sources:

        try:

            feed_articles = fetch_feed(source)

            for article in feed_articles:

                if article["id"] in existing_ids:
                    continue

                if not article["allow_ai_rewrite"]:
                    continue

                print(
                    f"Generating: "
                    f"{article['feed_title']}"
                )

                ai = generate_ai(article)

                published = (
                    ai["risk_level"] == "AUTO"
                )

                record = {
                    "id": article["id"],

                    "title": ai["headline_1"],

                    "alternative_headlines": [
                        ai["headline_2"],
                        ai["headline_3"]
                    ],

                    "summary": ai["summary"],

                    "deep_dive": ai["deep_dive"],

                    "category": article["category"],

                    "source": article["source"],

                    "source_url": article["source_url"],

                    "source_website": (
                        article["source_website"]
                    ),

                    "original_rss_title": (
                        article["feed_title"]
                    ),

                    "published_at": (
                        article["published"]
                    ),

                    "snippet24_status": (
                        "PUBLISHED"
                        if published
                        else ai["risk_level"]
                    ),

                    "risk_level": (
                        ai["risk_level"]
                    ),

                    "risk_reasons": (
                        ai["risk_reasons"]
                    ),

                    "created_at": (
                        datetime.now(
                            timezone.utc
                        ).isoformat()
                    )
                }

                new_articles.append(record)

                if len(new_articles) >= MAX_TOTAL_ARTICLES:
                    break

        except Exception as error:

            print(
                f"Source error "
                f"{source['name']}: {error}"
            )

    combined = new_articles + existing

    # Keep latest 500 articles.
    combined = combined[:500]

    save_json(
        ARTICLES_FILE,
        combined
    )

    print(
        f"Saved {len(new_articles)} new articles."
    )


if __name__ == "__main__":
    process()